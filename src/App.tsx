...
import WavesQuizGame from './components/WavesQuizGame';

function App() {
  const [gameState, setGameState] = useState<GameState>('start');
  const [selectedGame, setSelectedGame] = useState<'menu' | 'dragdrop' | 'waves'>('menu');
    ...
    return (
      <div className="min-h-screen bg-gray-100 flex flex-col justify-between items-center">
        <div className="flex-1 flex flex-col items-center justify-center w-full">
          <div className="bg-white rounded-lg shadow-lg p-8 max-w-md w-full flex flex-col gap-8 items-center">
            <h1 className="text-3xl font-bold text-center mb-4">Algebra Builder</h1>
            <button
              className="w-full bg-blue-600 text-white p-4 rounded-lg text-xl font-semibold hover:bg-blue-700 transition mb-2"
              onClick={() => {
                setSelectedGame('dragdrop');
                setGameState('start');
            ...
            <button
              className="w-full bg-green-600 text-white p-4 rounded-lg text-xl font-semibold hover:bg-green-700 transition"
              onClick={() => setSelectedGame('waves')}
            >
              Waves Quiz Game
            </button>
          </div>
  ...
  // Placeholder for Waves Quiz Game
  if (selectedGame === 'waves') {
    return <WavesQuizGame onBackToMenu={() => setSelectedGame('menu')} showCloseIcon />;
  }
        ...
        <div className="flex-1 flex items-center justify-center p-4">
          <div className="bg-white rounded-lg shadow-lg p-8 max-w-md w-full relative">
            ...
            <ul className="list-disc list-inside text-gray-700 space-y-3 mb-6 text-base">
              <li>Drag and drop each card into the correct category: Variable, Constant, Expression, or Equation.</li>
              <li>You can move cards between categories if you change your mind.</li>
              <li>Click Submit when you are done to see your results.</li>
            </ul>
  ...
  if (gameState === 'start') {
    return (
      <div className="min-h-screen bg-gray-100 flex flex-col relative">
        ...
        <div className="flex-1 flex items-center justify-center p-4">
          <div className="bg-white rounded-lg shadow-lg p-8 max-w-md w-full">
            <h1 className="text-3xl font-bold text-center mb-8">Algebra Builder</h1>
            ...
            <div className="grid gap-3" style={{ gridTemplateColumns: 'repeat(auto-fit, minmax(120px, 1fr))' }}>
              {availableNumbers.map((number) => (
                <div
                  ...
                  className={`number-card p-1 text-lg font-bold cursor-pointer select-none transition ring-2 ${selectedNumber === number ? 'ring-blue-500 bg-blue-50 scale-105' : 'ring-transparent bg-white'} hover:ring-blue-400 active:scale-95`}
